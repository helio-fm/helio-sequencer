...
void MidiLayer::sort()
{
	...
	{
		this->instances.sort(this->instances.getUnchecked(0));
	}
}
...
MidiLayer::Instance::Instance() : startBeat(0.f)
{
	Uuid uuid;
	id = uuid.toString();
}

MidiLayer::Instance::Instance(const MidiLayer::Instance &other) :
	startBeat(other.startBeat),
	id(other.id)
{
}

float MidiLayer::Instance::getStartBeat() const noexcept
{
	return this->startBeat;
}

String MidiLayer::Instance::getId() const noexcept
{
	return this->id;
}

XmlElement *MidiLayer::Instance::serialize() const
{
	auto xml = new XmlElement(Serialization::Core::layerInstance);
	xml->setAttribute("start", this->startBeat);
	xml->setAttribute("id", this->id);
	return xml;
}

void MidiLayer::Instance::deserialize(const XmlElement &xml)
{
	this->startBeat = float(xml.getDoubleAttribute("start", this->startBeat));
	this->id = xml.getStringAttribute("id", this->id);
}

void MidiLayer::Instance::reset()
{
	this->startBeat = 0.f;
}

int MidiLayer::Instance::compareElements(const MidiLayer::Instance &first,
	const MidiLayer::Instance &second)
{
...
Array<MidiLayer::Instance> &MidiLayer::getInstances() noexcept
{
	return this->instances;
}

bool MidiLayer::insertInstance(MidiLayer::Instance instance, const bool undoable)
{
	...
	if (undoable)
	{
		this->getUndoStack()->perform(new MidiLayerInsertInstanceAction(*this->getProject(),
			this->getLayerIdAsString(),
			instance));
	}
	else
	{
		this->instances.addSorted(instance, instance);
		//this->notifyEventAdded(*instance);
...
bool MidiLayer::removeInstance(MidiLayer::Instance instance, const bool undoable)
{
	if (undoable)
	{
		this->getUndoStack()->perform(new MidiLayerRemoveInstanceAction(*this->getProject(),
			this->getLayerIdAsString(),
			instance));
	}
	else
	{
		int index = this->instances.indexOfSorted(instance, instance);
		if (index >= 0)
		{
...
bool MidiLayer::changeInstance(MidiLayer::Instance instance, Instance newInstance, const bool undoable)
{
	if (undoable)
	{
		this->getUndoStack()->perform(new MidiLayerChangeInstanceAction(*this->getProject(),
			this->getLayerIdAsString(),
			instance,
			newInstance));
	}
	else
	{
		...
		{
			this->instances.remove(index);
			this->instances.addSorted(newInstance, newInstance);
